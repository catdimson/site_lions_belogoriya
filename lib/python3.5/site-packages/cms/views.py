...
from account.models import Sportsman
...
def details(request, slug):
    ...
    # custom
    # request.sportsman = Akk.objects.filter(is_user__username=request.user.username)
    try:
        request.us = User.objects.get(username=request.user.username)
        # request.sportsman = request.us.akk_set.all()
        print("Текущий пользователь в системе: ")
        print(request.us.username)
        request.account = Sportsman.objects.filter(name__username=request.us.username)
        print("Аккаунт: ")
